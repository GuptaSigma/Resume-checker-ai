...
class ResumeAnalyzer:
    def __init__(self, *, pdf_max_pages: int = 3):
        self.pdf_max_pages = pdf_max_pages
        self.skills_database: Dict[str, List[str]] = {
            'programming': ['python', 'java', 'javascript', 'c++', 'c#', 'php', 'ruby', 'go', 'rust', 'kotlin', 'swift', 'typescript'],
    ...
    def _extract_from_pdf(self, file_obj) -> str:
        if fitz is None:
            raise RuntimeError("PyMuPDF not installed. Please install with `pip install PyMuPDF`.")
        try:
            ...
            logger.warning(f"PDF processing with PyMuPDF failed: {e}. Returning placeholder text.")
    ...
    def extract_experience_months(self, text: str) -> int:
        """
        Calculates total experience in months by isolating the 'EXPERIENCE' section.
        """
        experience_text = ""
        ...
        for skill_name in found_skill_names:
            ...
            for cat, skills_list in self.skills_database.items():
                if skill_name in [s.lower() for s in skills_list]:
                    category = cat
                    break
            found_skills.append(SkillHit(name=skill_name.title(), category=category, match=True))
    ...
    def _count_roles(self, text: str) -> int:
        """Counts the number of distinct work experiences based on date ranges."""
        experience_text = ""
        section_pattern = re.compile(
            r'\b(EXPERIENCE|PROFESSIONAL EXPERIENCE|WORK EXPERIENCE)\b(.*?)(?=\b(?:PROJECTS|SKILLS|EDUCATION|LANGUAGES|ACTIVITIES)\b|\Z)',
            re.IGNORECASE | re.DOTALL
    ...
    def calculate_match_score(self, candidate_skills: List[SkillHit], target_skills: List[str],
        ...
        # NEW LOGIC: Bonus for number of matched skills
        ...
        # Internship, Hackathon, and Certifications
        # These will only add to the score if present. No negative scoring.
        if has_internship:
    ...
    def analyze_resume(self, file, target_role: str = "", jd_content: str = "", hr_requirements: str = "") -> Dict[str, Any]:
        try:
            ...
            final_target_role = target_role or self._predict_role_from_text(text)
            ...
            if experience_months == 0:
                ...
                exp_str = f"{experience_years} year" + ("s" if experience_years > 1 else "")
                if remaining_months > 0:
                     exp_str += f" {remaining_months} month" + ("s" if remaining_months > 1 else "")
                experience_display = exp_str
            else:
                experience_display = f"{experience_months} month" + ("s" if experience_months > 1 else "")
            ...
            match_score, match_reasons = self.calculate_match_score(
                skills, target_skills, experience_years, education, has_intern, has_hackathon, is_fresher_status, has_certifications_or_achievements, num_roles
            )
